    ...
    addToCart: (state, action) => {
      ...
      if (existingProduct) {
        ...
        if (existingProduct.quantity > 1)
          unitPrice = existingProduct.product_price / existingProduct.quantity;
        existingProduct.quantity += 1; // Increment the quantity if the product is already in the cart

        existingProduct.product_price = parseFloat(
          (existingProduct.product_price + unitPrice).toFixed(2)
        ); // Increment the price
      } else {
    ...
    removeFromCart: (state, action) => {
      ...
      if (existingProduct) {
        if (existingProduct.quantity > 1 && !remove) {
          ...
          const unitPrice =
            existingProduct.product_price / (existingProduct.quantity + 1);
